...
module.exports.search = function (item, callback) {

console.log("[DEBUG] [resume_db.search] Criteria:  %s", item);
  onConnect(function (err, connection) {
    r.db('resumes').table('resume').filter(item).pluck("user_name").run(connection, function(err, result) {
      if(err) {
        console.log("[ERROR][%s][saveResume] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err);
      }
...
module.exports.setup = function() {
  ...
  r.connect({host: dbConfig.host, port: dbConfig.port }, function (err, connection) {
	 ...
	 * when submit button is pressed, function reads what was searched, goes to the database to query a match, and lists a name as output
	*/

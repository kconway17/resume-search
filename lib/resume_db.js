...
module.exports.search = function (item, callback) {
	 ...
	 onConnect(function (err, connection) {
		 r.db('resumes').table('resume').filter(function(doc) {
		       var x = [];
		       for (var key in item) {
		              x.push(key);
		       }
		              var recursion =  function (keys, num) {
		                     if(!keys[num + 1]) {
		                           return doc(keys[num]).match("(?i)^" + item[keys[num]]);
		                     }
		                     return doc(keys[num]).match("(?i)^" + item[keys[num]] + '$').default(true).and(recursion(keys, num + 1));
		              }
		              if (x.length !== 0) {
